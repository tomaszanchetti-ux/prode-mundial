    ...
    return (
      <div
        className={`flex items-center gap-2 min-w-0 ${
          isWinner ? "text-text-primary font-medium" : "text-text-secondary"
        }`}
      >
        <TeamIdentity
          team={{
  ...
  return (
    ...
    >
      ...
      <div className="grid gap-1">
        <BracketSide side={match.home} isWinner={homeIsWinner} />
        <BracketSide side={match.away} isWinner={awayIsWinner} />
      </div>
    </button>
  );
}
  ...
  if (matches.length === 0) {
    return (
      <div className="flex flex-col gap-2 min-w-[180px]">
        <span className="typo-small text-text-muted sticky top-0 bg-surface-canvas py-1 z-[1]">{label}</span>
  ...
  return (
    <div className="grid gap-3">
      ...
      <div className="overflow-x-auto -mx-2 px-2">
        <section className="flex gap-3 min-w-max items-stretch">
          {columns.map((column) => (
            <RoundColumn
              key={column.key}
              label={column.label}
      ...
      {bronzeMatch ? (
        <section className="grid gap-2">
          <span className="typo-small text-text-muted">Tercer puesto</span>
