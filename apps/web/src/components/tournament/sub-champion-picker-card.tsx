...
export function SubChampionPickerCard({ data, championPick, onOpen }: SubChampionPickerCardProps) {
  const nowMs = useTicker();

  const requiresChampion = !championPick?.championTeamId;
  const statusMeta = resolveStatusMeta(data?.status);
  const currentPickId = data?.adjustedSubChampionTeamId ?? data?.subChampionTeamId ?? null;
  const ctaLabel = resolveCtaLabel(data?.status, requiresChampion);
