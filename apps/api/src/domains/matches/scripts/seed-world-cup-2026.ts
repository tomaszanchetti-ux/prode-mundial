...
function buildMatchSeed(match: WorldCup2026NormalizedMatch, nowIso: string): MatchSeed {
  ...
  const winnerTeamId =
    override && "winnerTeamId" in override ? override.winnerTeamId ?? null : null;
...
async function main() {
  ...
  console.log(`Seeded ${WORLD_CUP_2026_TEAMS.length} teams, ${WORLD_CUP_2026_GROUPS.length} groups and ${matches.length} matches.`);
}
